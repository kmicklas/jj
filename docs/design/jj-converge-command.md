...
than *N* commits (for change-id *B*) it stops the traversal and pretends that
the "boundary" commits (at the time traversal stops) are all successors of the
root commit. And finally, the graph contains a single start node which we call
the "*evolution fork point of B/0, B/1, ... , B/n*" (more on this below); any
commits "older" in the evolution graph than the evolution fork point are not
